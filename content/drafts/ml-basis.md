...
tags: []
draft: true
summary: "机器学习基础：线性模型、对数几率回归、线性判别分析、多分类学习等核心概念"
authors: ["default"]
---

[机器学习周志华](https://github.com/familyld/Machine_Learning)
...
$$
E(f;D) = \frac{1}{m} \sum*{i=1}^m(y_i-f(x_i))^2 = \frac{1}{m} \sum*{i=1}^m(y_i-wx_i-b)^2
$$
...
$$
w = \frac{\sum*{i=1}^m y_i(x_i - \bar{x})}{\sum*{i=1}^m x*i^2 - \frac{1}{m}(\sum*{i=1}^m x_i)^2}
$$
...
$$
E\_{\mathbf{\hat{w}}} = (\mathbf{y} - X\mathbf{\hat{w}})^T (\mathbf{y} - X\mathbf{\hat{w}})
$$
...
- XGBoost比GBDT新增内容：
  ...
  5. 近似直方图算法：采用加权分位数法来搜索近似最优分裂点
  6. 6.Shrinkage(缩减）：将学习到的模型\*系数，削减已学模型的权重
  7. 列采样：特征采样。
  8. 并行计算：特征预排序，特征分裂增益计算（均在特征粒度上）

- LightGBM比XGBoost新增内容（GBDT+GOSS+EFB）
